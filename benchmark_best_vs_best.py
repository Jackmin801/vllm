"""Fair comparison: each path uses its OWN best config. max_loras=1 and max_loras=4."""
...
with open("vllm/lora/ops/triton_ops/configs/E=8,N=4096,device_name=NVIDIA_B200,dtype=bfloat16.json") as f:
    fused_configs = {int(k): v for k, v in json.load(f).items()}
...
def run(num_tokens, max_loras, num_experts=8, top_k=2, K=4096, N=4096, rank=16,
    ...
    fc = nearest(fused_configs, num_tokens)
    ...
    for _ in range(n_warmup):
        ...
        invoke_fused_moe_lora_kernel(hidden, w, out_fused, topk_weights,
            ...
            True, top_k, 1, fc, compute_type=compute_type)
        # Separate warmup
        invoke_fused_moe_triton_kernel(hidden, w, out_base, None, None, topk_weights,
            sorted_b, expert_b, npost_b, True, top_k, bc, compute_type=compute_type,
            use_fp8_w8a8=False, use_int8_w8a8=False, use_int8_w8a16=False, use_int4_w4a16=False, per_channel_quant=False)
    ...
    # Bench separate (base + shrink + expand)
    torch.cuda.synchronize(); t0 = time.perf_counter()
    for _ in range(n_iter):
        out_base.zero_()
        invoke_fused_moe_triton_kernel(hidden, w, out_base, None, None, topk_weights,
    ...
    return base_ms, sep_ms, fused_ms

batch_sizes = [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 4096, 65536]

for ml in [1, 4]:
    print(f"\n{'='*90}")
    print(f"max_loras={ml} — each path uses its OWN best tuned config")
    print(f"{'='*90}")
    print(f"{'Tokens':>8s} | {'Base':>7s} | {'Sep':>7s} {'SepOH%':>7s} | {'Fused':>7s} {'FusOH%':>7s} | {'Winner':>10s}")
    print("-" * 75)
    for M in batch_sizes:
        nw, ni = (5, 15) if M >= 16384 else (5, 25) if M >= 4096 else (8, 40)
        b, s, f = run(M, ml, n_warmup=nw, n_iter=ni)
        soh = (s-b)/b*100
        foh = (f-b)/b*100
        winner = "FUSED" if f < s else "SEPARATE"
        print(f"{M:>8d} | {b:>6.3f}  | {s:>6.3f}  {soh:>+6.1f}% | {f:>6.3f}  {foh:>+6.1f}% | {winner:>10s}")
        sys.stdout.flush()
